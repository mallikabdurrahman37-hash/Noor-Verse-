...
/** Boot: first-visit popup OR apply saved preference immediately */
function initI18n() {
  const saved = localStorage.getItem('nv_lang'); // null on first visit

  if (saved) {
    /* Returning visitor — apply silently, never show popup */
    applyLanguage(saved);
  } else {
    /* First visit — show the selection popup */
    showFirstVisitPopup();
  }

  /* Wire up the in-menu lang-option buttons (modal-language) */
  document.querySelectorAll('.lang-option-btn').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  /* Wire up first-visit popup buttons */
  document.querySelectorAll('.fv-lang-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const code = btn.dataset.fvLang;
      applyLanguage(code);           // apply i18n strings
      hideFirstVisitPopup();         // smooth close
    });
  });
}

/** Show the first-visit overlay with entrance animation */
function showFirstVisitPopup() {
  const overlay = document.getElementById('lang-first-visit-overlay');
  if (!overlay) return;

  overlay.style.display = 'flex';   // make it layout-visible
  ...
  const firstBtn = overlay.querySelector('.fv-lang-btn');
  if (firstBtn) firstBtn.focus();
}

/** Hide the first-visit overlay with exit animation */
function hideFirstVisitPopup() {
  const overlay = document.getElementById('lang-first-visit-overlay');
  if (!overlay) return;

  overlay.classList.add('fv-hiding');
  overlay.classList.remove('fv-visible');

  overlay.addEventListener('transitionend', () => {
    overlay.style.display = 'none';
    overlay.classList.remove('fv-hiding');
  }, { once: true });
}
